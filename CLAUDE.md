...
- Global listeners live in five hooks plus one scoped site: useSync (online/visibilitychange/interval), useDragReorder (window, during a drag), useOutsidePress (document pointerdown, mounted in Shell), useEnterToCreate (document keydown, mounted in Shell), DialogFrame (document keydown while a dialog is open; ConfirmDialog and SettingsDialog build on it), and useSystemDark (a matchMedia change listener, in App and while SettingsDialog is mounted).
