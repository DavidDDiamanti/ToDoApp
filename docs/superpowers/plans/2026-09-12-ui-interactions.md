...
- [x] Tests first (TodoTree.test, new `describe('hover')`). Body of an item: `screen.getByRole('treeitem', { name: 'Alpha' }).querySelector('[data-item-body]')` (Task 1 adds `data-item-body` to the `.body` div; Task 2 reuses it).
...
- [x] CSS: the ≤480 px rule becomes `.item[data-active='true'] > .body > .row, .item[data-hovered='true'] > .body > .row { flex-wrap: wrap; }`.
- [x] `npm test && npm run typecheck`; commit `feat: reveal item actions on hover`.
...
- [x] Shell imports the renamed hook. `npm test && npm run typecheck`; commit `feat: presses off an item deselect it; presses on items never close the editor`.
...
- [x] Tests first. Hook test: exactly one `keydown` listener on document, same handler removed. EditorFlow (`renderApp` also mounts `useEnterToCreate`; use `fireEvent.keyDown(document.body, { key: 'Enter' })` or `userEvent.keyboard('{Enter}')` after focusing the target):
...
- [x] `openAddUnder(id)` in actions.ts (import `useUiStore` and `editorKindFor` from `./uiStore`):
  ```ts
...
- [x] Hook:
  ...
  ```
- [x] Title button in TodoItem: `onKeyDown={(e) => { if (e.key !== 'Enter' || e.altKey || e.ctrlKey || e.metaKey || e.shiftKey) return; e.preventDefault(); useUiStore.getState().setActive(todo.id); openAddUnder(todo.id); }}` with a comment: preventing the keydown suppresses the synthesized click, so Enter does not also toggle details; Space still does.
- [x] Shell mounts the hook next to `useOutsidePress`. `npm test && npm run typecheck`; commit `feat: Enter opens a new item, or a child of the selected item`.
...
- [x] Tests first. uiStore.test: `resolvePending` after `requestOpen(edit a)`, dirty, `requestOpen(root)` opens root and clears dirty and the prompt; `resolvePending` with no prompt is a no-op. TodoEditor.test (prompt now named "Unsaved changes"; the submit-labelled button is inside the dialog, so query `within(dialog)`):
...
- [x] Store: `resolvePending: () => { const { pendingClose } = get(); if (pendingClose === null) return; set({ openEditor: pendingClose.next, editorDirty: false, pendingClose: null }); }` and `confirmDiscard: () => get().resolvePending()`.
- [x] TodoEditor: `function trySave(): boolean` holds the current `submit` body minus `closeEditor()` and returns false on an empty title (error set, title focused); `submit` = `if (trySave()) closeEditor()`. Dialog:
  ```tsx
  <ConfirmDialog
...
- [x] `npm test && npm run typecheck`; commit `feat: the unsaved-changes prompt can save`.
...
- [x] Spec addendum `## 14. v4 interactions (2026-09-12)` with subsections: hover reveal; selection and Enter (routing table); outside presses (supersedes the outside-press bullets of 13.1); the prompt (supersedes "Discard changes?"); spacing.
- [x] README usage paragraph updated (≤520 words, no dashes). CLAUDE.md listener line updated. This plan's boxes ticked.
- [x] `npm test && npm run typecheck && npm run build`. Commit `docs: v4 interactions`.
