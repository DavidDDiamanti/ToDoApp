...
4. Enable the Email auth provider, add `http://localhost:5173/**` to the redirect URLs, and set the Site URL to the same address. Add `{{ .Token }}` to the Magic Link email template so it includes the six digit sign-in code.
...
- `src/domain/tree.ts` and `src/domain/ops.ts` hold pure domain logic. The tree builds a children map from the flat list; orphans sit at the top level. Ops returns every operation as patches: the completion cascade, and delete with promotion via fractional `sort_order`. Moves are guarded against cycles.
...
- `src/sync/syncEngine.ts` and `src/sync/supabaseRemote.ts` push dirty rows in chunks of 200, pull changes with a 60 second overlap, and take live Realtime updates. Conflicts resolve by newest `updated_at`, guarding against overwriting a dirty local row. A trigger in `0001_todos.sql` rejects stale writes, RLS scopes rows to their owner, and a generation counter ignores syncs after sign out.
...
Two offline devices editing the same item resolve by wall clock time; a parent completed while a child reopens offline can sit mixed until the next click. A deploy reloads the page via the service worker, losing unsaved editor text; dashboard deletes are ignored, since it only soft deletes. Sync calls have no timeout, so a hung request blocks the cycle until it fails.
